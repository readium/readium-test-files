...
printg "Now verifying that the commit hashes are the same for each readium-cfi-js submodule -------"
echo "${green}Comparing cf-js submodules${NC}"

cd $ROOT_PWD/readium-js-viewer$SUFFIX/readium-js/readium-shared-js/readium-cfi-js
echo_pwd
git log -n 1 --pretty=format:"%H" $BRANCH

cd $ROOT_PWD/SDKLauncher-Android$SUFFIX/readium-shared-js/readium-cfi-js
echo_pwd
git log -n 1 --pretty=format:"%H" $BRANCH

cd $ROOT_PWD/SDKLauncher-iOS$SUFFIX/Resources/readium-shared-js/readium-cfi-js
echo_pwd
git log -n 1 --pretty=format:"%H" $BRANCH

cd $ROOT_PWD/SDKLauncher-OSX$SUFFIX/readium-shared-js/readium-cfi-js
echo_pwd
git log -n 1 --pretty=format:"%H" $BRANCH
